import { c as createLucideIcon, e as useParams, r as reactExports, f as useRegions, g as useRegionPrices, j as jsxRuntimeExports, L as Link, d as Skeleton, C as ChartColumn, H as House } from "./index-CA7MdyxF.js";
import { P as PropertyType, e as formatMonth, B as Button, b as formatPrice, C as Card, c as CardHeader, d as CardTitle, a as CardContent, g as formatPriceCompact } from "./card-BC1EZS11.js";
import { M as MapPin, a as PriceChangeIndicator, P as PROPERTY_TYPE_LABELS, T as TrendingUp, b as TrendingDown } from "./types-Klw--uXm.js";
import { R as ResponsiveContainer, L as LineChart, C as CartesianGrid, X as XAxis, Y as YAxis, a as ReferenceLine, T as Tooltip, b as Line } from "./LineChart-Gddju51D.js";
/**
...
function RegionDetail() {
  ...
  const chartData = sorted.map((p) => ({
    ...
    평균가격: Number(p.averagePrice) / 1e4,
    rawAvg: p.averagePrice,
    rawChange: p.changePercent
  }));
  const chartMin = priceValues.length > 0 ? Math.floor(minPrice / 1e4 * 0.97 * 10) / 10 : 0;
  const chartMax = priceValues.length > 0 ? Math.ceil(maxPrice / 1e4 * 1.03 * 10) / 10 : 10;
          ...
          children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              YAxis,
              {
                ...
                tickFormatter: (v) => `${formatPriceCompact(v * 1e4)}`,
                tick: {
                  fontSize: 11,
                  fill: "oklch(var(--muted-foreground))"
                },
